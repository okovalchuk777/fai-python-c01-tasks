DROP DATABASE IF EXISTS Homework06_20230610;
CREATE DATABASE Homework06_20230610;
USE Homework06_20230610;
...
-- Создайте таблицу users_old, аналогичную таблице users. Создайте процедуру, с помощью которой можно переместить 
-- любого (одного) пользователя из таблицы users в таблицу users_old. 
-- (использование транзакции с выбором commit или rollback – обязательно).

DROP PROCEDURE IF EXISTS retired_users;
DELIMITER //
CREATE PROCEDURE retired_users(retired_user_id INT)
BEGIN
	BEGIN
	...
	END;
    START TRANSACTION;
    SET @old_id = (SELECT id FROM users WHERE id = retired_user_id);
    SET @old_firstname = (SELECT firstname FROM users WHERE id = retired_user_id);
    SET @old_lastname = (SELECT lastname FROM users WHERE id = retired_user_id);
    SET @old_email = (SELECT email FROM users WHERE id = retired_user_id);
    
    INSERT INTO users_old (id, firstname, lastname, email)
	VALUES (@old_id, @old_firstname, @old_lastname, @old_email);
    
    SET SQL_SAFE_UPDATES = 0;
	DELETE FROM users WHERE id=retired_user_id;
